 ...
 * [01_evaluate_generator.py](./src/01_evaluate_generator.py): A demonstration of training a GPT-2 model to generate JS programs. 
 * [02_evaluate_mutator.py](./src/02_evaluate_mutator.py): A demonstration of test program mutation. This demonstrates how we mutate the test JS programs generated by the previous demonstration.
 * [03_evaluate_harness.py](./src/03_evaluate_harness.py): A demonstration of our differential testing approach. The results from the prior steps are used to perform differential testing on one JS testbed (engine).
 * [04_coverage_calculate.py](./src/04_coverage_calculate.py): A demonstration of the code coverage statistics, showing the quality of the test cases generated by different fuzzers.
...
For convience, we have provided the test data generated by our fuzzers. But you can still check [this document](./prior-fuzzers.md)  for how to use other fuzzers (CodeAlchemist, DeepSmith, Fuzzilli, Montage, DIE) for test program generation and differential testing. 
This evaluation requires using a larger docker image (80+GB uncompressed) that can be downloaded from [here](https://drive.google.com/file/d/1WMd4GMin5rOke40CaFwylm34ddmBpFGY/view?usp=sharing)
