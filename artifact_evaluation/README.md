...
Replace ```comfort_generate``` in option ```--coverage_files=/root/data/codeCoverage/totalFiles/comfort_generate``` to ```codealchemist_generate```, ```deepsmith_generate```, ```die_generate```, ```fuzzilli_generate```, ```montage_generate``` to compute the coverage on the 10,000 test programs generated by other fuzzers in Figure 8. 
