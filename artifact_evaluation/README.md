...
*Optional*: You can use the following command to test the code coverage on 10,000 Comfort-generated test programs for longer run (6+ hours):
  ...
  Replace ```comfort_generate``` in ```--coverage_files=/root/data/codeCoverage/totalFiles/comfort_generate``` to ```codealchemist_generate```, ```deepsmith_generate```, ```die_generate```, ```fuzzilli_generate```, ```montage_generate``` to compute the coverage on the 10,000 test programs generated by other fuzzers in Figure 8. 
