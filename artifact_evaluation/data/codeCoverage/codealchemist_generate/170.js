...
var v1 = (function (v1, Promise){
return Promise.resolve().then((function (){
return v1.schema.createTable('publishedProjects', (function (v3){
v3.increments('id');
v3.text('title').notNullable();
v3.text('tags');
v3.text('description');
}));
})).then((function (){
return v1.schema.createTable('users', (function (v3){
v3.increments('id');
v3.text('name').notNullable().unique();
}));
})).then((function (){
return v1.schema.createTable('projects', (function (v3){
v3.increments('id');
v3.integer('user_id').notNullable().references('id').inTable('users').onDelete('CASCADE');
v3.integer('published_id').references('id').inTable('publishedProjects');
v3.text('title').notNullable();
v3.text('tags');
v3.text('description');
v3.text('publish_url');
v3.text('date_created').notNullable();
v3.text('date_updated').notNullable();
}));
})).then((function (){
return v1.schema.createTable('files', (function (v3){
v3.increments('id');
v3.integer('project_id').notNullable().references('id').inTable('projects').onDelete('CASCADE');
v3.text('path').notNullable();
v3.specificType('buffer', 'bytea').notNullable();
}));
})).then((function (){
return v1.schema.createTable('publishedFiles', (function (v3){
v3.increments('id');
v3.integer('published_id').notNullable().references('id').inTable('publishedProjects').onDelete('CASCADE');
v3.integer('file_id').references('id').inTable('files');
v3.text('path').notNullable();
v3.specificType('buffer', 'bytea').notNullable();
}));
}));
...
return (v3) instanceof (Date);
}
// GenBlkBrick
for(var v4 = 0;(v4) < (0x8000);v4++){
var v5 = (function (v1){
...
var v7 = (function (v1){
return /^debug_/i.test(v1);
});
var v8 = (function (v1, v2, v3){
Array.prototype.reduce.call(arguments, v1, v6);
});
}
// GenBlkBrick
while((v9) !== (0)){
...
(v10.ClearCache) = (function (){
(v1) = [];
});
(v10.Serialize) = (function (v2){
var v3 = ({});
...
(v10.SerializeMesh) = (function (v19, v20, v21){
if((v20) === (void 0)){
(v20) = false;
}
if((v21) === (void 0)){
